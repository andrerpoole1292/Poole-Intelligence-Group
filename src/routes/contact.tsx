        ...
        <section className="bg-gradient-to-b from-white via-[#F5F7FA] to-white pt-32 pb-20 dark:from-gray-950 dark:via-[#0A1628]/20 dark:to-gray-950">
          <div className="mx-auto max-w-4xl px-6 text-center">
            ...
            <h1 className="text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl">
              Let's Start a <span className="bg-gradient-to-r from-[#1B2A4A] to-[#3A5A8C] bg-clip-text text-transparent dark:from-[#6B8DBF] dark:to-[#3A5A8C]">Conversation</span>
            </h1>
            ...
            <div className="rounded-2xl border border-[#E8E8E8] bg-[#F5F7FA] p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:p-12">
              <div className="flex flex-col items-center py-12 text-center">
                ...
                <div className="mt-6 w-full max-w-sm rounded-xl border border-[#C0C0C0] bg-white p-5 dark:border-gray-700 dark:bg-gray-950">
                  ...
                  <a href={CALENDLY_LINK} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex w-full items-center justify-center rounded-lg bg-[#1B2A4A] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#2A3A5A]">Schedule a Free Discovery Call</a>
                </div>
                <button onClick={() => setSubmitted(false)} className="mt-6 rounded-lg border border-[#C0C0C0] bg-white px-6 py-3 text-sm font-semibold text-[#1B2A4A] transition hover:bg-[#F5F7FA] dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:hover:bg-gray-800">Send Another Message</button>
              </div>
            </div>
          </div>
        </section>
                  ...
                  <div className="space-y-6">
                    <div className="grid gap-6 sm:grid-cols-2">
                      <div>
                        ...
                        <input type="text" id="name" name="name" required placeholder="Jane Smith" className="w-full rounded-lg border border-[#C0C0C0] bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 transition focus:border-[#1B2A4A] focus:outline-none focus:ring-2 focus:ring-[#1B2A4A]/20 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:border-[#6B8DBF]" />
                      </div>
                      <div>
                        ...
                        <input type="email" id="email" name="email" required placeholder="jane@company.com" className="w-full rounded-lg border border-[#C0C0C0] bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 transition focus:border-[#1B2A4A] focus:outline-none focus:ring-2 focus:ring-[#1B2A4A]/20 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:border-[#6B8DBF]" />
                      </div>
                    </div>
                    <div className="grid gap-6 sm:grid-cols-2">
                      <div>
                        <label htmlFor="company" className="mb-2 block text-sm font-medium text-[#1B2A4A] dark:text-gray-100">Company</label>
                        <input type="text" id="company" name="company" placeholder="Acme Corp" className="w-full rounded-lg border border-[#C0C0C0] bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 transition focus:border-[#1B2A4A] focus:outline-none focus:ring-2 focus:ring-[#1B2A4A]/20 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:border-[#6B8DBF]" />
                      </div>
                      <div>
                        <label htmlFor="service" className="mb-2 block text-sm font-medium text-[#1B2A4A] dark:text-gray-100">Service interest</label>
                        <select id="service" name="service" className="w-full rounded-lg border border-[#C0C0C0] bg-white px-4 py-3 text-sm transition focus:border-[#1B2A4A] focus:outline-none focus:ring-2 focus:ring-[#1B2A4A]/20 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:focus:border-[#6B8DBF]">
                    ...
                    <div>
                      ...
                      <textarea id="message" name="message" required rows={5} placeholder="Tell us about your AI goals, challenges, or any questions you have..." className="w-full rounded-lg border border-[#C0C0C0] bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 transition focus:border-[#1B2A4A] focus:outline-none focus:ring-2 focus:ring-[#1B2A4A]/20 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder-gray-500 dark:focus:border-[#6B8DBF]" />
                    </div>
                    {error && (
                      <div className="rounded-lg bg-red-50 p-4 text-sm text-red-700 dark:bg-red-950/50 dark:text-red-400">{error}</div>
                    )}
                    <button type="submit" disabled={sending} className="w-full rounded-xl bg-[#1B2A4A] px-8 py-4 text-sm font-semibold text-white shadow-lg shadow-[#1B2A4A]/25 transition hover:bg-[#2A3A5A] disabled:cursor-not-allowed disabled:opacity-60">
            ...
            <div className="lg:col-span-2">
              <div className="rounded-2xl border border-[#E8E8E8] bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-950">
                <div className="mb-6 rounded-xl bg-gradient-to-br from-[#1B2A4A] to-[#3A5A8C] p-6 text-center">
                  ...
                  <a href={CALENDLY_LINK} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-white px-6 py-3 text-sm font-semibold text-[#1B2A4A] shadow-sm transition hover:bg-gray-100">Schedule a Free Discovery Call</a>
              ...
              <div className="mt-6 rounded-2xl border border-[#E8E8E8] bg-[#F5F7FA] p-8 dark:border-gray-800 dark:bg-gray-900">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Drop us a line at <a href="mailto:hello@pooleintelligencegroup.com" className="font-medium text-[#1B2A4A] underline hover:text-[#3A5A8C] dark:text-[#6B8DBF] dark:hover:text-[#8BAED4]">hello@pooleintelligencegroup.com</a>
                </p>
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                  Or follow us on <a href="https://linkedin.com/company/pooleintelligence" target="_blank" rel="noopener noreferrer" className="font-medium text-[#1B2A4A] underline hover:text-[#3A5A8C] dark:text-[#6B8DBF] dark:hover:text-[#8BAED4]">LinkedIn</a> for practical AI insights and case studies.
                </p>
              </div>
            </div>
          </div>
